...
def test(model, device, dataloader, num_classes, class_balanced_acc=False):
    ...
    return result
#%%
def test_rand(model, device, dataloader, num_classes, noise_norm, class_balanced_acc=False):
    model.eval()#set model to evaluation mode
    sample_count=0
    adv_sample_count=0
    sample_idx_wrong=[]
...
#%%
def clip_normA_(noise, norm_type, norm_max, X=None, X_range=(0,1)):
    ...
    with torch.no_grad():
        ...
        #-----------
        if X is not None:
            Xnew = torch.clamp(X+noise, X_range[0], X_range[1])
            noise -= noise-(Xnew-X)
        #-----------
    return noise
#%%
...
def pgd_attack(model, X, Y, noise_norm, norm_type=np.inf, max_iter=None, step=None,
    ...
    #flag[k]=0, otherwise (wrongly classified if targerted is False)
    #-----------------
    if rand_init == True:
        init_value=rand_init_norm
        if rand_init_norm is None:
    ...
    for n in range(0, max_iter+1):
        ...
        if stop_if_label_change == True:
            ...
            if no_attack_on_wrong == True:
                Xn[Yp_e_Y] = X[Yp_e_Y] + noise_new[Yp_e_Y]
            else:
                Xn = X+noise_new
        #---------------------------
...
def repeated_pgd_attack(model, X, Y, noise_norm, norm_type, max_iter, step,
    ...
    for m in range(0, num_repeats+1):
        Xm, Zm, Ypm = pgd_attack(model, X, Y, noise_norm, norm_type=norm_type, max_iter=max_iter, step=step,
                                 ...
                                 targeted=targeted, clip_X_min=clip_X_min, clip_X_max=clip_X_max,
                                 stop_if_label_change=True, use_optimizer=use_optimizer,
...
def test_adv(model, device, dataloader, num_classes, noise_norm, norm_type, max_iter, step, method,
             targeted=False, clip_X_min=0, clip_X_max=1, use_optimizer=True, num_repeats=0,
    ...
    for batch_idx, (X, Y) in enumerate(dataloader):
        ...
        #------------------
        if method == 'ifgsm':
            Xn, Zn, Ypn = ifgsm_attack(model, X, Y, noise_norm=noise_norm, norm_type=norm_type,
                                       max_iter=max_iter, step=step, targeted=targeted,
                                       clip_X_min=clip_X_min, clip_X_max=clip_X_max,
                                       use_optimizer=use_optimizer, loss_fn=loss_fn, return_output=True)
        elif method == 'pgd':
            Xn, Zn, Ypn = repeated_pgd_attack(model, X, Y, noise_norm=noise_norm, norm_type=norm_type,
                                              max_iter=max_iter, step=step, targeted=targeted,
                                              clip_X_min=clip_X_min, clip_X_max=clip_X_max,
                                              use_optimizer=use_optimizer, loss_fn=loss_fn,
